...
1. **카테고리** — 버그 / 개선 제안 / 질문 (객관식)
2. **대상 스킬** — craft-landing / report-issue / 전체·기타 (객관식)
3. **컨텍스트 자동 추출** — 현재 대화에서 의도·실제 결과·재현 방법·에러 추출 (다시 물어보지 X)
4. **Issue 초안 미리보기 + 컨펌** — 마크다운 형식으로 완성된 issue 보여줌. user 수정 가능
5. **Issue 생성** — `gh issue create`로 등록 후 URL 반환
